...
for subject_id in subject_ids:
    ...
    try:
        ...
        assert pure_V.shape == cont_V.shape == (19, target_n_times), f"Shape mismatch for subject {subject_id} after resampling"
        assert veog_V.shape == heog_V.shape == (target_n_times,), f"EOG shape mismatch for subject {subject_id} after resampling"
        ...
        print(f"    - Most affected channel: {stats['most_affected_channels'][0]['channel']} (RMS: {stats['most_affected_channels'][0]['rms']:.2e} V)")
...
summary_data = {
    ...
    'resampling_info': {
        ...
        'subjects_resampled': [sub_id for sub_id, results in subject_results.items() if results.get('resampled', False)],
        'subjects_not_resampled': [sub_id for sub_id, results in subject_results.items() if not results.get('resampled', False)]
    }
}
