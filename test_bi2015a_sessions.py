    ...
    try:
        ...
        if 'session' in metadata.columns:
            ...
            if n_splits == len(sessions):
                print("✓ SUCCESS: LeaveOneGroupOut correctly identifies all sessions!")
            else:
                print(f"✗ WARNING: Expected {len(sessions)} folds, but got {n_splits}")
            ...
            print("\n✗ ERROR: No 'session' column found in metadata")
...
if __name__ == "__main__":
    ...
    if all_passed:
        ...
        print("\n✗ Some datasets failed to load. Please review the output above.")
        sys.exit(1)
