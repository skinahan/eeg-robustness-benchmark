#!/usr/bin/env python3
"""
Systematic evaluation of HYDRAv2 features vs branched_wiredcfc_arch4 baseline.

This script:
1. Tests a baseline HYDRAv2 configuration (all features disabled, closest to branched_wiredcfc_arch4)
2. Systematically enables each feature individually
3. Tests feature combinations (Phase 1, Phase 2, Phase 3 as per hydra_spec.txt)
4. Compares all results to determine which changes help vs. hurt

Features tested:
- Baseline: All HYDRAv2 features disabled (num_attn_queries=1, all heads/context/skip disabled)
- Individual features:
  * Adaptive residual gating (use_adaptive_residual=True)
  * Multi-query pooling (num_attn_queries=4, attn_dropout=0.1)
  * ERP evidence head (use_erp_head=True)
  * SSVEP evidence head (use_ssvep_head=True)
  * Cross-bin context (use_cross_bin_context=True)
  * Global skip path (use_global_skip=True)
- Phase combinations:
  * Phase 1: Adaptive residual + Multi-query (highest leverage per spec)
  * Phase 2: Phase 1 + ERP + SSVEP heads
  * Phase 3: Phase 2 + Cross-bin context + Global skip
  * Full: Default HYDRAv2 configuration

The baseline configuration uses:
- num_attn_queries=1 (single query, like original TemporalAttnPool)
- use_adaptive_residual=False (uses global backwards_rezero like branched_wiredcfc_arch4)
- All evidence heads disabled
- No cross-bin context
- No global skip

This allows us to isolate the impact of each feature and determine which improvements
are actually helping vs. hurting performance.
"""
...
def register_hydra_v2_variant(variant_name: str, wiring, feature_config: Dict):
    """
    Register a HYDRAv2 variant with specific feature configuration.
    ...
    Args:
        variant_name: Name for the variant (e.g., "hydra_v2_baseline")
...
def define_feature_configs():
    """
    Define all feature configurations to test.
    ...
    # Baseline: All features disabled (as close to branched_wiredcfc_arch4 as possible)
    configs["hydra_v2_baseline"] = {
        'num_attn_queries': 1,  # Single query (like original TemporalAttnPool)
        'attn_dropout': 0.0,  # No dropout
        'use_adaptive_residual': False,  # Use global residual (backwards_rezero)
        'use_erp_head': False,
    ...
    # Individual features
    configs["hydra_v2_adaptive_residual"] = {
        'num_attn_queries': 1,
        'attn_dropout': 0.0,
        'use_adaptive_residual': True,  # Enable adaptive residual gating
        'use_erp_head': False,
        'use_ssvep_head': False,
        'use_cross_bin_context': False,
        'use_global_skip': False,
    ...
    configs["hydra_v2_multi_query"] = {
        'num_attn_queries': 4,  # Multi-query pooling
        'attn_dropout': 0.1,
    ...
    configs["hydra_v2_erp_head"] = {
        'num_attn_queries': 1,
        'attn_dropout': 0.0,
        'use_adaptive_residual': False,
        'use_erp_head': True,  # Enable ERP head
        'use_ssvep_head': False,
        'use_cross_bin_context': False,
        'use_global_skip': False,
    }
    
    configs["hydra_v2_ssvep_head"] = {
        'num_attn_queries': 1,
        'attn_dropout': 0.0,
        'use_adaptive_residual': False,
        'use_erp_head': False,
        'use_ssvep_head': True,  # Enable SSVEP head
        'use_cross_bin_context': False,
        'use_global_skip': False,
    }
    
    configs["hydra_v2_cross_bin_context"] = {
        'num_attn_queries': 1,
        'attn_dropout': 0.0,
        'use_adaptive_residual': False,
        'use_erp_head': False,
        'use_ssvep_head': False,
        'use_cross_bin_context': True,  # Enable cross-bin context
        'use_global_skip': False,
    }
    
    configs["hydra_v2_global_skip"] = {
        'num_attn_queries': 1,
        'attn_dropout': 0.0,
        'use_adaptive_residual': False,
        'use_erp_head': False,
        'use_ssvep_head': False,
        'use_cross_bin_context': False,
        'use_global_skip': True,  # Enable global skip
    }
    
    # Phase 1 combination: Adaptive residual + Multi-query (highest leverage)
    configs["hydra_v2_phase1"] = {
        'num_attn_queries': 4,
        'attn_dropout': 0.1,
    ...
    # Phase 2 combination: Phase 1 + ERP + SSVEP heads
    configs["hydra_v2_phase2"] = {
        'num_attn_queries': 4,
        'attn_dropout': 0.1,
        'use_adaptive_residual': True,
        'use_erp_head': True,
    ...
    # Phase 3 combination: Phase 2 + Cross-bin context + Global skip
    configs["hydra_v2_phase3"] = {
        'num_attn_queries': 4,
        'attn_dropout': 0.1,
        'use_adaptive_residual': True,
        'use_erp_head': True,
    ...
    # Full HYDRAv2 (default configuration)
    configs["hydra_v2_full"] = {
        'num_attn_queries': 4,
        'attn_dropout': 0.1,
        'use_adaptive_residual': True,
        'use_erp_head': True,
        'use_ssvep_head': True,
        'use_cross_bin_context': False,  # Default is False in create_hydra_v2_classifier
        'use_global_skip': True,
    }
    
    return configs

def run_experiment(model, script_dir, output_file, dataset="BNCI2014_001", subjects=[1], 
    ...
    try:
        ...
        print(f"[OK] {model} completed successfully")
        return 0
    except subprocess.CalledProcessError as e:
        print(f"[FAILED] {model} failed with error code {e.returncode}")
        # Print last 30 lines of output for quick reference
        try:
...
def write_summary_to_file(script_dir, all_dataset_results, output_file):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("=" * 80 + "\n")
        f.write("HYDRAv2 SYSTEMATIC FEATURE EVALUATION - COMPREHENSIVE SUMMARY\n")
        f.write("=" * 80 + "\n\n")
...
def main():
    """Run the systematic feature evaluation."""
    ...
    print("=" * 80)
    print("HYDRAv2 SYSTEMATIC FEATURE EVALUATION")
    print("=" * 80)
    ...
    print("\nRegistering HYDRAv2 variants...")
    registered_variants = []
    for variant_name, config in feature_configs.items():
        try:
            register_hydra_v2_variant(variant_name, wiring, config)
    ...
    for dataset_name, dataset_info in datasets_config.items():
        ...
        for model in models_to_test:
            output_file = os.path.join(script_dir, f"test_hydra_comparison_{model}_{dataset_name}_output.txt")
            output_files[model] = output_file
    ...
    summary_output_file = os.path.join(script_dir, "test_hydra_comparison_summary.txt")
    print("\n" + "=" * 80)
    ...
    if all_passed_all_datasets:
        ...
        for dataset_name, dataset_info in all_dataset_results.items():
            print(f"\n  {dataset_name}:")
            for model, output_file in dataset_info['output_files'].items():
                print(f"    {model}: {output_file}")
    else:
        print("\n[FAILED] Some tests failed. Check output files and summary for details.")
